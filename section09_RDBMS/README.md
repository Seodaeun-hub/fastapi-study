...
direct_get_conn()은 단순히 connection 객체를 반환하는 함수이지, context manager 프로토콜(__enter__와 __exit__)을 구현하지 않았기 때문이다. with 문법은 자동으로 리소소를 정리해야되므로 __enter__와 __exit__가 필수이다. 즉 @contextmanager를 쓰면 이 프로토콜이 자동으로 구현돼서 with를 쓸 수 있는 것이다.
